...
var DatatableJsonRemoteCreens = function () {
    ...
    var demo = function () {
        ...
        datatable = $('.m_datatable').mDatatable({
            ...
            data: {
                type: 'remote',
                source: '/Outlet/GetDataOutletUser',
                pageSize: 10,
            },
            ...
            columns: [

                {
                    ...
                    template: function (row, index, datatable) {
                        ...
                        return '\
						<a href="/Outlet/UpdateOutletStoreUser?id='+ row.id + '" class="m-portlet__nav-link btn m-btn m-btn--hover-danger m-btn--icon m-btn--icon-only m-btn--pill" title="Update" target="_blank">\
                ...
                },
                {
                    ...
                    template: function (row, index, datatable) {
                        if (row.updated != null) {
                            var updated = {
                                true: { 'title': 'Đã sửa', 'class': 'm-badge--brand' },
                                false: { 'title': 'Chưa sửa', 'class': ' m-badge--danger' },
                            };
                            return '<span class="m-badge ' + updated[row.updated].class + ' m-badge--wide">' + updated[row.updated].title + '</span>';
                         }
                        return "";
                    }
                },
        ...
        $('#m_form_city').on('change', function () {
            if ($(this).val() === "") {
                ...
                datatable.search(change($('#m_form_city option:selected').text()).toUpperCase(), "province");
                setTimeout(function () {
                    $('#m_form_district').change();
                }, 1000);
            }
        ...
        $('#m_form_User').on('change', function () {
            if ($(this).val() === "") {
                datatable.search("", "updatedBy");
            }
            else {
                datatable.search($(this).val(), "updatedBy");
            }
        }).val(typeof query.Type !== 'undefined' ? query.Type : '');
        $('#m_form_city, #m_form_district,#m_form_Area,#m_form_User').selectpicker();
    ...
    $(".m-datatable__table").on('dblclick', 'tbody tr', function () {
        ...
        if (outlet != null) {
            ...
            outlet.projectDetail.forEach(function (projetDetail) {
                ...
                html += temp.replace("{1}", projetDetail.costPayForDrugStore);
                html += temp.replace("{1}", projetDetail.costPayForProduction);
