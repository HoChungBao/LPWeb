        ...
        var toggleHiddenColumns = function (e) {
          ...
          if (!$(detailRow).hasClass('m-datatable__row-detail')) {
            ...
            $(hidden).each(function () {
              $(detailSubTable).
              append($('<tr class="m-datatable__row"></tr>').append($('<td class="m-datatable__cell"></td>').append($('<span/>').css('width', Plugin.offset).append($(this).data('field')))).append(this));
            });
  ...
  $.fn.mWizard = function (options) {
    ...
    var Plugin = {
      ...
      init: function (options) {
        ...
        wizard.btnCustomNext = wizard.find('[data-wizard-action="customnext"]');
      ...
      build: function () {
        ...
        //== Next button custom btnCustomNext handler
          wizard.btnCustomNext.on('click',
              function(e) {
                  e.preventDefault();
                  if (Plugin.getNextStep() === 3) {
                      if (noNeedFiles) {
                          Plugin.goTo(4);
                          getVal();
                          return;
                      ...
                      if (!isUploadContract) {
                        var des = "Vui Lòng Bấm Vào Nút Upload(Tài liệu) trước khi sang bước tiếp theo";
                        swal("Upload Giấy tờ liên quan", des, "warning");
                        return;
                      }
                      if (!isUploadVideo) {
                          var des = "Vui Lòng Bấm Vào Nút Upload(Video) trước khi sang bước tiếp theo";
                          swal("Upload Video", des, "warning");
                          return;
                      }
                  }

                  if (Plugin.getNextStep() === 4) {
                      getVal();
                  }
...
$(document).ready(function () {
  $('.js-containUpload').append('<div class="col-lg-12 m--margin-top-15 listImg"></div>');
  $('.js-containUpload').append('<div class="col-lg-12 m--margin-top-15 listDoc"></div>');

  /**
   * Detect files' type
   * @param {Node} input 
   */
  function detectFilesType(input) {
    ...
    if (Files) {
      ...
      var imgsVar = [];
      var docsVar = [];
      var imgsEx = ["jpg", "jpeg", "png", "bmp"];

      for (i = 0; i < numFiles; i++) {
        extension = listFiles[i].name.substring(listFiles[i].name.lastIndexOf('.') + 1);
        if (imgsEx.indexOf(extension) > -1)
          imgsVar.push(listFiles[i]);
        else
          docsVar.push(listFiles[i]);
      }

      var parent = input.closest('.js-containUpload');

      var imgContainer = $(parent).find('.listImg');
      var docContainer = $(parent).find('.listDoc');
      if (imgsVar.length > 0)
        insertImg(imgsVar, imgContainer);

      if (docsVar.length > 0)
        insertDoc(docsVar, docContainer);

    }
  }

  /**
   * Insert into img Array if file's type is image
   * @param {Array} list 
   * @param {Node} container 
   */
  function insertImg(list, container) {
    ...
    if (list) {
      ...
      for (i = 0; i < numFiles; i++) {
        ...
        reader.onload = function (event) {
          $($.parseHTML('<img class="listImg__item">')).attr('src', event.target.result).appendTo(container);

          //console.log(event.target.result);
        }
        reader.readAsDataURL(list[i]);
      }
      $('.listImg').css('display', 'unset');
    }
  }
  /**
   * Others file's type
   ...
   * @param {Node} container 
   */
  function insertDoc(list, container) {
    var numFiles = list.length;
    $(container).empty();
    if (list) {
      for (i = 0; i < numFiles; i++) {
        $($.parseHTML('<p class="listDoc__text">_' + list[i].name + ' - ' + list[i].size + ' kb</p>')).appendTo(container);
      }
      $('.listDoc').css('display', 'unset');
    }
  }

  $('.js-showUpload').on('change', function () {
    detectFilesType(this);
  });
  ...
  $('.js-dropdown-type').on('change', function() {
    var type = $(this).val();
      ...
      if(type == 1) {
        $('.hospital-specialty').removeClass('d-none');
        $('.hospital-city').removeClass('d-none');
        $('.pharmacy-city').addClass('d-none');
        $('.pharmacy-district').addClass('d-none');
      } else if(type == 2) {
        $('.hospital-specialty').addClass('d-none');
        $('.hospital-city').addClass('d-none');
        $('.pharmacy-city').removeClass('d-none');
        $('.pharmacy-district').removeClass('d-none');
      } else {
        $('.hospital-specialty').addClass('d-none');
        $('.hospital-city').addClass('d-none');
        $('.pharmacy-city').addClass('d-none');
