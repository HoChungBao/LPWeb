...
namespace LienPhatERP.Services
{
    public static class ServiceHelpers
    {
        ...
        {
            return Utilities.StringUtils.CreateUrl(name, "-");
        }

        public static string GenerateSlug(string stringToSlug, IEnumerable<dynamic> similarList, string previousSlug)
        {
            // url generator
            var slug = CreateUrl(stringToSlug);
            slug = StringUtil.RemoveSign4VietnameseString(slug);
